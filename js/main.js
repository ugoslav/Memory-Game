...
//const calculateNumberOfPics = () => {
//    let numberOfNumbers = 0;
//    numberOfNumbers = numberOfNumbers + Math.ceil(numberOfNumbers / 5);
//};
...
function newOutroMessage() {
    ...
    headingSpace.appendChild(outroText);
    setTimeout(() => {
        window.location.href = "../html/playAgain.html";
    },3000
)
}
...
function newLevelCompleteMessage()
{
    ...
    setTimeout(() => {
        if(parseInt(sessionStorage.getItem("LEVELNUMBER")) < 5)
            window.location.href = "../html/transition.html";
        else
            window.location.href = "../html/gameComplete.html";
    },3000);
}
...
const changeImg = (image) => {
    if((turnedCards.includes(parseInt(image.id)) === false) && (closedCards.includes(parseInt(image.id)) === false))
    {
        ...
        if(img.id <= 5)
            img.src = `../imgs/pic${img.id}.jpg`;
        else
            img.src = `../imgs/pic${img.id - 5}.jpg`;
...
//let flipRemaining = Math.floor(images.length - (images.length / 3));
let flipRemaining;
